...
public interface InitiativeDao extends BaseDao<Initiative> {
    ...
    @Query("INSERT OR IGNORE INTO initiative (id,name,target_date,description,target_area,location," +
            "imagen,target_amount,created_by,ref_code) VALUES(:id,:name,:target_date,:description,:location," +
            ":imagen,:target_amount) ")
    long insert(long id, String name, String target_date, String description, String location,
                Bitmap imagen, String target_amount);
    ...
    @Query("SELECT * FROM initiative WHERE id IN (SELECT id_initiative FROM userJoinInitiative WHERE user_email=:userEmail AND type=:type AND is_deleted=:is_deleted)")
    ...
    default void upsert(Initiative obj) {
        long id = insert(obj.getId(),obj.getName(),obj.getTarget_date(),obj.getDescription(),obj.getLocation(),
                obj.getImagen(),obj.getTarget_amount());
        if (id == -1) {
